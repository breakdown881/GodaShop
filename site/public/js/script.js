...
$(function(){
  ...
  // Hiển thị carousel for relative products
    ...
    $("form.form-comment").submit(function(event) {
        event.preventDefault(); //ngăn chặn submit lên server chuyển qua trang mới
        var post_url = $(this).attr("action"); //get form action url
        var request_method = $(this).attr("method"); //get form GET/POST method
        var form_data = $(this).serialize(); //Encode form elements for submission
    ...
    $("header form.header-form .search").keyup(function(event) {
        / Act on the event /
        clearTimeout(timeout); //xóa đi, rồi tính lại
    ...
    $("main .district").change(function(event) {
     ...
     // Validate form information account
    $("main form.info-account").submit(function(event) {
        return checkRetypePassword(this);
    });
});
...
function getUpdatedParam($param, $value) {
    ...
    for (p in params) {
        ...
        //x[2] = 'sort=alphabet'
        x.push(p + "=" + params[p]);
        //x.push: thêm 1 phần tử vào cuối array
    }
    return str_param = x.join("&");//c=product&c=list&sort=alphabet
    //x.join("&"): nối các phần tử bằng dấu "&"
...
function displayCart(data) {
    var cart = JSON.parse(data); //chuyển chuỗi thành object
    var total_product_number = cart.total_product_number;
    $(".btn-cart-detail .number-total-product").html(total_product_number);

    var total_price = cart.total_price;
    $("#modal-cart-detail .price-total").html(number_format(Math.round(total_price))+"₫");
    var items = cart.items;
    var rows = "";
    console.log(items);
    for (let i in items) {
        //let: tầm vực truy xuất cục bộ, nằm trong for
        let item = items[i];
        var row = '<hr>'+
            '<div class="clearfix text-left">'+
                '<div class="row">'+
                    ...
                    '<div class="col-sm-6 col-md-3"><input type="hidden" value="'+item.qty+'"><input type="number" onchange="updateProductInCart(this,'+item.product_id+')" min="1" value="'+item.qty+'"></div>'+
...
function checkRetypePassword(form) {
    var passwords = $(form).find('input[type=password]');
    if (passwords.length > 1 && 
        $(passwords[0]).val() !== $(passwords[1]).val()) {
        $(passwords[1]).after("<div class='alert alert-danger'>Password không khớp</div>");
        return false;
    }
    return true;
}